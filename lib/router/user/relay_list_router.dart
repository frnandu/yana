...
class _RelayListRouter extends State<RelayListRouter>
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title:
            ...
            Text(
                "${relayList!.displayTitle} (${relayList!.allRelays!.length})"),
      ),
      body: Container(
        ...
        child: RefreshIndicator(
          onRefresh: () async {
            Nip51List? refreshedList = await ndk.lists.getSingleNip51List(
                relayList!.kind, loggedUserSigner!,
                forceRefresh: true);
          ...
          child: ListView.builder(
                ...
                onRemove: (url) async {
                  ...
                  if (result != null && result) {
                    EasyLoading.show(
                        ...
                        dismissOnTap: true);
                    relayList = await ndk.lists.broadcastRemoveNip51Relay(
                        relayList!.kind, url, myOutboxRelaySet!.urls,
            ...
            itemCount: (relayList != null ? relayList!.allRelays!.length : 0) +
  ...
  void onEditingComplete() async {
    List<String> result = await relayProvider.findRelays(controller.text,
        nip: relayList!.kind == Nip51List.kSearchRelays ? Nip50.kNip : null);
    result.forEach((url) {
  ...
  Future<void> add(String url, bool private) async {
    ...
    bool? result = await ConfirmDialog.show(context,
        "Confirm add ${private ? "private" : "public"} ${url} to list");
    if (result != null && result) {
      EasyLoading.show(
          status: 'Broadcasting relay list...',
          maskType: EasyLoadingMaskType.black,
          dismissOnTap: true);
      relayList = await ndk.lists.broadcastAddNip51ListRelay(
          relayList!.kind, url, myOutboxRelaySet!.urls,
          private: private);
      if (relayList!.kind == Nip51List.kSearchRelays) {
        searchRelays = relayList!.allRelays!;
