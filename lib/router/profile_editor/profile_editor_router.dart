...
class _ProfileEditorRouter extends CustState<ProfileEditorRouter> {
  ...
  Widget doBuild(BuildContext context) {
    ...
    return FutureBuilder<Metadata?>(future: Future(() async {
      Metadata? cached = metadata ??
          await metadataProvider.getMetadata(loggedUserSigner!.getPublicKey());
      if (cached != null &&
          cached.refreshedTimestamp != null &&
          cached.refreshedTimestamp! >
              (DateTime.now()
                      .subtract(REFRESH_METADATA_BEFORE_EDIT_DURATION)
                      .millisecondsSinceEpoch ~/
                  1000)) {
        metadata = cached;
      } else {
        EasyLoading.show(
            status: "Refreshing metadata from relays...",
            maskType: EasyLoadingMaskType.black,
            dismissOnTap: true);
        await ndk.metadata
            .loadMetadata(loggedUserSigner!.getPublicKey(), forceRefresh: true)
            .then(
          (metadata) {
          ...
          metadata ??= Metadata(
              pubKey: loggedUserSigner!.getPublicKey(), updatedAt: Helpers.now);
          EasyLoading.dismiss();
          return metadata;
        });
      }
  ...
  Future<List<String?>> pickImageAndUpload() async {
    ...
    for (final filepath in filepaths) {
      if (filepath != null) {
        ...
        final userServerList = await ndk.blossomUserServerList
            .getUserServerList(pubkeys: [loggedUserSigner!.getPublicKey()]);
  ...
  void profileSave() async {
    ...
    await ndk.metadata
        .broadcastMetadata(metadata!, specificRelays: myOutboxRelaySet!.urls);
